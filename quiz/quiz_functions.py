...
def get_scored_users(target_quiz_uuid):
    ...
    target_answers = UserAnswers.objects.filter(quiz=target_quiz)
    correctly_answered_users = target_answers.filter(choice=target_quiz.question.correctChoice).values_list("user", flat=True)
    incorrectly_answered_users = target_answers.exclude(choice=target_quiz.question.correctChoice).values_list("user", flat=True)
    scored_users = {
...
def users_add_score(target_event_id, target_users_uuid, points, inc_correctNums_flg=True):
    ...
    event_joined_users = UserScores.objects.filter(event=target_event_id)
    
    for target_uuid in target_users_uuid:
        try:
            target_user = event_joined_users.get(user=target_uuid)
        ...
        target_user.score += points
        if inc_correctNums_flg:
            target_user.correctNums += 1
        target_user.save()
...
def update_ranking(target_event_id):
    event_joined_users = UserScores.objects.filter(event=target_event_id).order_by("-score")
